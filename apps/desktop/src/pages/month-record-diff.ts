...
type MonthRecordDiffFieldKey = Extract<
  keyof UpsertEmployeeMonthRecordPayload,
  keyof EmployeeMonthRecord
>;

export type MonthRecordDraftDiff = {
  field: MonthRecordDiffFieldKey;
  label: string;
  currentValue: EmployeeMonthRecord[MonthRecordDiffFieldKey];
  draftValue: UpsertEmployeeMonthRecordPayload[MonthRecordDiffFieldKey];
};
...
const diffFields: Array<{
  ...
  { key: "supplementarySalaryIncome", label: "补发收入" },
  { key: "supplementaryWithheldTaxAdjustment", label: "补扣税调整" },
  { key: "supplementarySourcePeriodLabel", label: "补发所属期间" },
  { key: "supplementaryRemark", label: "补发备注" },
