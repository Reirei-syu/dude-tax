...
const buildDetailLines = (summary: AnnualTaxRuleSourceSummary) => {
  ...
  if (summary.usedPreviousYearIncomeReference) {
    lines.push(
      `已参考上一年度收入口径；上一年度收入不超过 6 万元的判断为：${summary.previousYearIncomeUnder60k ? "是" : "否"}。`,
    );
  }

  if (summary.usedFirstSalaryMonthReference) {
    lines.push(
      `已参考年度首次发薪月份，当前判定的首次发薪月为 ${summary.firstSalaryMonthInYear ?? "未识别"} 月。`,
    );
  }
...
export const buildAnnualTaxRuleSourceExplanation = (
  ...
  if (!summary) {
    return {
      title: "规则来源",
      summary: "当前结果未写入额外规则来源摘要，默认按当前单位 / 年份自身数据解释。",
  ...
  return {
    title: usedExternalRule ? "当前结果引用了外部规则上下文" : "当前结果仅使用本单位数据",
    summary: usedExternalRule
      ? "该结果不是完全基于当前单位孤立数据计算，系统已按预扣规则引用了跨单位或跨年度上下文。"
