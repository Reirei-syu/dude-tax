import { DEFAULT_BASIC_DEDUCTION_AMOUNT } from "../../../../packages/config/src/index";
import type {
  Employee,
  EmployeeMonthRecord,
...
export const MonthRecordEntryPage = () => {
  ...
  const loadMonthRecords = async (employeeId: number) => {
    ...
    try {
      ...
      setErrorMessage(null);
      setNoticeMessage(null);
      const nextRecords = await apiClient.listMonthRecords(
        currentUnitId,
        currentTaxYear,
        employeeId,
      );
      ...
      setForm(
        toMonthRecordPayload(
          nextRecords.find((record) => record.taxMonth === nextSelectedMonth) ?? null,
        ),
      );
    } catch (error) {
    ...
    return (
      <section className="page-grid">
        ...
        <article className="glass-card page-section placeholder-card">
          {" "}
          <h1>月度数据录入</h1> <p>请先在顶部选择单位和年份，再进入月度数据录入模块。</p>{" "}
        </article>{" "}
      </section>
    );
  }
        ...
        {selectedEmployee ? (
          <div className="summary-grid detail-summary-grid">
            ...
            <div className="summary-card">
              {" "}
              <span>已完成月份</span> <strong>{monthSummary.completed}</strong>{" "}
            </div>{" "}
            <div className="summary-card">
              {" "}
              <span>待补录月份</span> <strong>{monthSummary.draft}</strong>{" "}
            </div>{" "}
            <div className="summary-card">
              {" "}
              <span>未开始月份</span> <strong>{monthSummary.notStarted}</strong>{" "}
            </div>{" "}
            <div className="summary-card">
              {" "}
              <span>已录入月份</span> <strong>{monthSummary.recorded}</strong>{" "}
            </div>{" "}
            <div className="summary-card">
              {" "}
              <span>完成度</span> <strong>{monthSummary.completionRate}%</strong>{" "}
            </div>{" "}
            <div className="summary-card">
              {" "}
              <span>批量草稿月份</span> <strong>{draftMonthCount}</strong>{" "}
            </div>{" "}
            <div className="summary-card">
              {" "}
              <span>差异月份</span> <strong>{draftDiffSummary.changedMonths.length}</strong>{" "}
            </div>{" "}
          </div>
        ) : null}{" "}
        {selectedEmployee ? (
          <div className="batch-edit-panel">
            ...
            <div className="section-header">
              ...
              <div>
                ...
                <h3>批量编辑</h3>{" "}
                <p>
                  勾选目标月份后，可将右侧当前表单内容批量应用为本地草稿，不会立即保存到数据库。
                </p>{" "}
              </div>{" "}
              ...
              <section className="year-quarter-card" key={quarter.quarter}>
                ...
                <div className="year-quarter-header">
                  ...
                  <div>
                    ...
                    <p>
                      {" "}
                      已完成 {quarter.summary.completed} / 待补录 {quarter.summary.draft} / 未开始{" "}
                      {quarter.summary.notStarted}{" "}
                    </p>{" "}
                  </div>{" "}
                  ...
                  {quarter.months.map((month) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          {" "}
                          <strong>{month.taxMonth} 月</strong> <p>状态：{month.statusLabel}</p>{" "}
                          <p>
                            {" "}
                            工资：{month.salaryIncome.toFixed(2)} / 预扣税：
                            {month.withheldTax.toFixed(2)}{" "}
                          </p>{" "}
                        </div>{" "}
