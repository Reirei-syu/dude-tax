import { annualTaxWithholdingModeLabelMap, type AnnualTaxWithholdingSummary } from "@dude-tax/core";

export { annualTaxWithholdingModeLabelMap };
...
export const buildAnnualTaxWithholdingExplanation = (
  ...
  if (summary.withholdingMode === "annual_60000_upfront") {
    return {
      ...
      detailLines: ["在累计收入未超过 6 万元前，规则应预扣税额可能为 0。", ...commonDetailLines],
    };
  }
  ...
  return {
    ...
    detailLines: ["累计减除费用按已处理月份数乘以月减除费用计算。", ...commonDetailLines],
  };
};
