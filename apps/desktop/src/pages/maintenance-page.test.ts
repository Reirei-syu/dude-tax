...
test("系统维护页移除模块总览卡片并改为独立页头", () => {
  assert.equal(maintenancePageSource.includes('cardId="maintenance-header"'), false);
  assert.equal(maintenancePageSource.includes("maintenance-page-header"), false);
  assert.equal(maintenancePageSource.includes("当前税率版本"), false);
  assert.equal(maintenancePageSource.includes("版本数量"), false);
  assert.equal(maintenancePageSource.includes("编辑状态"), false);
});
...
test("系统维护页顶层工作区卡片从页面顶部开始排布", () => {
  assert.equal(
    maintenancePageSource.includes('cardId="maintenance-tax"\n          defaultLayout={{ x: 0, y: 0, w: 12, h: 24 }}'),
    true,
  );
  assert.equal(
    maintenancePageSource.includes('cardId="maintenance-versions"\n          defaultLayout={{ x: 0, y: 24, w: 12, h: 18 }}'),
    true,
  );
  assert.equal(
    maintenancePageSource.includes('cardId="maintenance-impact"\n          defaultLayout={{ x: 0, y: 42, w: 12, h: 16 }}'),
    true,
  );
});
