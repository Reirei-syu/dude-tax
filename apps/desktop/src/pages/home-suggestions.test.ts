...
import type { EmployeeCalculationStatus, ImportSummary } from "../../../../packages/core/src/index";
...
test("有导入冲突时优先提示处理冲突", () => {
  const suggestions = buildHomeSuggestions({
    ...
    pendingRecalculateCount: 0,
    invalidatedCount: 0,
    importSummary: createImportSummary({ conflictRows: 3 }),
    statuses: [createStatus()],
  });

  assert.equal(suggestions[0]?.title, "先处理导入冲突");
  assert.equal(suggestions[0]?.path, "/import");
});

test("有可导入数据且无员工时优先提示继续导入", () => {
  const suggestions = buildHomeSuggestions({
    currentUnitId: 1,
    currentTaxYear: 2026,
    employeeCount: 0,
    ...
    pendingRecalculateCount: 0,
    invalidatedCount: 0,
    importSummary: createImportSummary({ readyRows: 6 }),
    statuses: [],
  });

  assert.equal(suggestions[0]?.title, "执行批量导入");
  assert.equal(suggestions[1]?.title, "先补员工基础档案");
});
