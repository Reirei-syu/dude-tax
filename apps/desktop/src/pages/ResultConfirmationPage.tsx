...
export const ResultConfirmationPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setErrorMessage(error instanceof Error ? error.message : "加载缴纳确认页面失败");
      } finally {
        setLoading(false);
      }
    };
  ...
  const downloadWorkbook = async () => {
    ...
    try {
      ...
      await saveFileWithDesktopFallback({
        defaultPath: `缴纳确认_${currentUnit.unitName}_${currentTaxYear}_${selectedMonth}月.xlsx`,
        filters: [{ name: "Excel 文件", extensions: ["xlsx"] }],
        mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        content: workbookArray,
      });
      setNoticeMessage("缴纳确认导出已生成。");
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "导出缴纳确认失败");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (!currentUnitId || !currentTaxYear) {
    return (
      <WorkspaceLayoutRoot scope="page:result-confirmation">
        <WorkspaceCanvas>
          <WorkspaceItem cardId="result-confirmation-main" defaultLayout={{ x: 0, y: 0, w: 12, h: 10 }} minH={8}>
            <CollapsibleSectionCard
              cardId="result-confirmation-main"
              className="placeholder-card"
              description="请先在顶部选择单位和年份，再进入缴纳确认模块。"
              headingTag="h1"
              title="缴纳确认"
            />
          </WorkspaceItem>
      ...
      <WorkspaceCanvas>
        ...
        >
          <CollapsibleSectionCard
            cardId="result-confirmation-main"
            className="placeholder-card"
            description="请选择已纳税月份并确认"
            headingTag="h1"
            headerExtras={<span className="tag">{loading ? "加载中" : "缴纳确认"}</span>}
            title="已纳税月份确认"
          >
