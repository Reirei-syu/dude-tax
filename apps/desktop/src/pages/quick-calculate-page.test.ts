...
test("快速计算页隐藏首次取得工资累计选项并显示另一方案税额", () => {
  assert.equal(
    quickCalculatePageSource.includes('mode !== "first_salary_month_cumulative"'),
    true,
  );
...
test("快速计算页在年终奖单独计税方案下展示年终奖税额和税率", () => {
  ...
  assert.equal(quickCalculatePageSource.includes('result.selectedScheme === "separate_bonus"'), true);
