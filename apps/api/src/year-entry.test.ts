...
test("年度工作台保存会阻断入职前和离职后收入月份，确认后才允许保存", async () => {
  const [{ registerYearEntryRoutes }, , , { unitRepository }, { employeeRepository }] =
    await modulesPromise;

  const app = Fastify({ logger: false });
  await registerYearEntryRoutes(app);

  const unit = unitRepository.create({ unitName: "就业月份冲突测试单位", remark: "" });
  const employee = employeeRepository.create(unit.id, {
    employeeCode: "EMP-CONFLICT-001",
    employeeName: "冲突员工",
  ...
  const blockedResponse = await app.inject({
    ...
    payload: {
      months: [
        {
          taxMonth: 6,
          ...createMonthRecordPayload({ salaryIncome: 8_000 }),
        },
        {
          taxMonth: 10,
          ...createMonthRecordPayload({ otherIncome: 1_500 }),
        },
      ],
    },
  });
  ...
  assert.deepEqual(blockedBody.conflictMonths, [6, 10]);
  assert.deepEqual(blockedBody.beforeHireMonths, [6]);
  assert.deepEqual(blockedBody.afterLeaveMonths, [10]);
  assert.equal(blockedBody.hireDate, "2026-07-01");
  assert.equal(blockedBody.leaveDate, "2026-09-30");
  ...
  const acknowledgedResponse = await app.inject({
    ...
    payload: {
      ...
      acknowledgedEmploymentConflictMonths: [6, 10],
    },
  });

  assert.equal(acknowledgedResponse.statusCode, 200);
  const workspace = acknowledgedResponse.json() as { hireDate: string | null; leaveDate: string | null };
  assert.equal(workspace.hireDate, "2026-07-01");
  assert.equal(workspace.leaveDate, "2026-09-30");
