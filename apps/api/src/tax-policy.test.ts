...
test("读取税率接口返回当前配置与默认配置", async () => {
  ...
  assert.equal((body.currentSettings as Record<string, unknown>).basicDeductionAmount, 5_000);
  assert.equal(
    ((body.currentSettings as Record<string, unknown>).comprehensiveTaxBrackets as unknown[])
      .length,
    7,
  );
...
test("仅保存说明时不应清空年度结果与重算记录", async () => {
  ...
  monthRecordRepository.upsert(unit.id, employee.id, 2026, 1, createMonthRecordPayload());
...
test("绑定单位年度作用域后，仅该作用域结果切换到绑定版本语义", async () => {
  ...
  assert.equal((bindBody.currentSettings as Record<string, unknown>).basicDeductionAmount, 5_000);
...
test("保存税率时支持自定义版本名称", async () => {
  ...
  const response = await app.inject({
    ...
    url: "/api/tax-policy",
    payload: {
      basicDeductionAmount: 6_500,
      versionName: "2026 春季专项版",
      comprehensiveTaxBrackets: [
        { level: 1, maxAnnualIncome: 36_000, rate: 3, quickDeduction: 0 },
        { level: 2, maxAnnualIncome: 144_000, rate: 10, quickDeduction: 2520 },
        { level: 3, maxAnnualIncome: 300_000, rate: 20, quickDeduction: 16920 },
  ...
  assert.equal(response.statusCode, 200);
  const body = response.json() as Record<string, unknown>;
  assert.equal(body.currentVersionName, "2026 春季专项版");
  const versions = body.versions as Array<Record<string, unknown>>;
  assert.equal(
    versions.some((version) => version.versionName === "2026 春季专项版"),
    true,
  );
...
test("可重命名已有税率版本并写入审计日志", async () => {
  ...
  const currentBody = currentResponse.json() as Record<string, unknown>;
  const versionId = Number(currentBody.currentVersionId);

  const renameResponse = await app.inject({
    method: "PATCH",
    url: `/api/tax-policy/versions/${versionId}`,
    payload: {
      versionName: "2026 重命名测试版",
    },
  });

  assert.equal(renameResponse.statusCode, 200);
  const renameBody = renameResponse.json() as Record<string, unknown>;
  assert.equal(renameBody.currentVersionName, "2026 重命名测试版");
  const auditLogs = renameBody.auditLogs as Array<Record<string, unknown>>;
  assert.equal(auditLogs[0]?.actionType, "rename_version");
  assert.equal(String(auditLogs[0]?.summary).includes("重命名"), true);

  await app.close();
});

test("带作用域参数重命名版本时会同步版本列表与作用域绑定名称", async () => {
  const [{}, { registerTaxPolicyRoutes }, { unitRepository }] = await modulesPromise;

  const app = Fastify({ logger: false });
  await registerTaxPolicyRoutes(app);

  const unit = unitRepository.create({
    unitName: "税率作用域改名测试单位",
    remark: "",
  });
  ...
  const bindResponse = await app.inject({
    method: "POST",
    url: `/api/tax-policy/versions/${versionId}/bind-scope`,
    payload: {
      unitId: unit.id,
      taxYear: 2026,
    },
  ...
  assert.equal(renameResponse.statusCode, 200);
  const renameBody = renameResponse.json() as Record<string, unknown>;
  assert.equal(renameBody.currentVersionName, "2026 作用域重命名版");
  const versions = renameBody.versions as Array<Record<string, unknown>>;
  assert.equal(
    versions.some((version) => version.versionName === "2026 作用域重命名版"),
    true,
  );
  const currentScopeBinding = renameBody.currentScopeBinding as Record<string, unknown>;
  assert.equal(currentScopeBinding.versionName, "2026 作用域重命名版");
  const auditLogs = renameBody.auditLogs as Array<Record<string, unknown>>;
  assert.equal(auditLogs[0]?.actionType, "rename_version");
  assert.equal(String(auditLogs[0]?.summary).includes("作用域重命名版"), true);

  const scopedQueryResponse = await app.inject({
    method: "GET",
    url: `/api/tax-policy?unitId=${unit.id}&taxYear=2026`,
  });
  assert.equal(scopedQueryResponse.statusCode, 200);
  const scopedQueryBody = scopedQueryResponse.json() as Record<string, unknown>;
  assert.equal(scopedQueryBody.currentVersionName, "2026 作用域重命名版");
  assert.equal(
    (scopedQueryBody.currentScopeBinding as Record<string, unknown>).versionName,
    "2026 作用域重命名版",
  );
